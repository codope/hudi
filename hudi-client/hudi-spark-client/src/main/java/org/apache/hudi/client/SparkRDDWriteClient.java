...
public class SparkRDDWriteClient<T extends HoodieRecordPayload> extends
  ...
  protected void completeCompaction(HoodieCommitMetadata metadata,
    ...
    try {
      ...
      if (basePath.contains(".hoodie/metadata")) {
        killJVMIfDesired("/tmp/fail32_mt_compaction.txt", "Fail metadata table compaction", 4);
      } else {
        killJVMIfDesired("/tmp/fail62_dt_compaction.txt", "Fail data table compaction", 4);
      }
  ...
  private void completeClustering(HoodieReplaceCommitMetadata metadata,
    ...
    try {
      ...
      if (basePath.contains(".hoodie/metadata")) {
        killJVMIfDesired("/tmp/fail31_mt_clustering.txt", "Fail metadata table clustering", 4);
      } else {
        killJVMIfDesired("/tmp/fail61_dt_clustering.txt", "Fail data table clustering", 4);
      }
