...
public abstract class AbstractTableFileSystemView implements SyncableFileSystemView, Serializable {
  ...
  // RFC-111 / HUDI-1045 (Phase 0a): guard for writes silently dropped because they landed in a file
    ...
    fileGroups.stream().collect(Collectors.groupingBy(HoodieFileGroup::getPartitionPath))
        .forEach((partition, value) -> {
          if (!isPartitionAvailableInStore(partition)) {
            ...
            // RFC-111 / HUDI-1045 (Phase 0a): flag any write silently dropped by hard replacement.
  ...
  /**
   * RFC-111 / HUDI-1045 (Phase 0a) guard. For each just-built file group that has been replaced by a
    ...
    for (HoodieFileGroup fileGroup : fileGroups) {
      ...
      String msg = String.format(
          "Detected potential lost write: file group %s was replaced by instant %s but still has a "
              + "live file slice newer than %s; such slices are excluded from queries (RFC-111/HUDI-1045).",
  ...
  /**
   * RFC-111 / HUDI-1045 (Phase 0a): count of lost-write detections since this view was created.
  ...
  /**
   * RFC-111 / HUDI-1045 (Phase 0a): when enabled, a detected lost write throws
